...
//         'gemini-2.5-pro': true,
//         'gemini-1.5-flash': true,
//         // Do NOT add 'image-generation-005' here, as it's a separate flow
//     });
...
//         try {
//             const data = await callBackendApi('/summarize', { prompt, selectedModels, imageData }); // Pass imageData
//             setResults({ summaries: data });
//         } catch (error) {
//             setResults({ error: error.message });
//         } finally {
...
//         try {
//             const data = await callBackendApi('/expand', { prompt, selectedModels, imageData }); // Pass imageData
//             setResults({ expansions: data });
//         } catch (error) {
//             setResults({ error: error.message });
//         } finally {
...
//         try {
//             // Call the NEW backend endpoint for image generation
//             const data = await callBackendApi('/generate-image', { prompt: imageGenPrompt });
//             if (data && data.imageUrl) {
//                 setGeneratedImageUrl(data.imageUrl); // Set the received Base64 image URL
//             } else {
//                 setResults({ error: 'Image generation failed or returned no image URL.' });
//             }
//         } catch (error) {
//             setResults({ error: error.message });
//         } finally {
...
function App() {
    ...
    // NEW: Function to generate an image using the backend's Imagen endpoint
    const generateImage = async () => {
        if (!imageGenPrompt.trim()) {
            setResults({ error: 'Please enter a prompt for image generation.' });
            return;
