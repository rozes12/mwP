...
function App() {
    ...
    const handleSelectChat = async (chatId) => {
        try {
            const response = await fetch(`${BACKEND_BASE_URL}/chats/${chatId}`);
            const data = await response.json();
    ...
    const runPrompt = async () => {
        ...
        try {
            ...
            const newChatResponse = await fetch(`${BACKEND_BASE_URL}/chats`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ prompt, results: responseResults })
            });
    ...
    const handleImageGenPromptChange = (e) => setImageGenPrompt(e.target.value);
    const handleModelToggle = (modelName) => setSelectedModels(prev => ({ ...prev, [modelName]: !prev[modelName] }));
    const handleImagenModelSelect = (e) => setSelectedImagenModel(e.target.value);
    const handleImageChange = (event) => {
        const file = event.target.files[0];
        if (file) {
            setSelectedImage(file);
